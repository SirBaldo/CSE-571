...
class ParticleFilter:
    ...
    def move_particles(self, env: Field, u):
        ...
        for i in range(self.num_particles):
            ...
            new_particles[i] = env.forward(self.particles[i], u_noisy).ravel()
    ...
    def update(self, env: Field, u, z, marker_id):
        ...
        mean, cov = None, None
        # YOUR IMPLEMENTATION HERE
        weight = np.zeros(self.num_particles)
        for i in range(self.num_particles):
            z_observation = env.observe(particles[i], marker_id)
            difference = minimized_angle(z- z_observation)
            weight[i] = env.likelihood(difference, self.beta)

        weight = weight/np.sum(weight)
        self.particles = self.resample(particles, weight)
        self.weights = np.ones(self.num_particles)/self.num_particles
